...
namespace ACT.Core.Services
{
    public class BaseService<T> : IDisposable, IEntity<T> where T : class
    {
        ...
        /// <summary>
        /// Gets a list of T matching the specified search params
        /// </summary>
        /// <param name="pm"></param>
        /// <param name="csm"></param>
        /// <returns></returns>
        public virtual List<T> ListCSM(PagingModel pm, CustomSearchModel csm)
        {
            ...
            string tName = myType.Name.ToString();

            query = string.Format(@"{0} SELECT
                            p.*
                             FROM
                                [dbo].[{1}] p ", query, tName);
            // WHERE
            ...
            return context.Database.SqlQuery<T>(query, parameters.ToArray()).ToList();
        }
